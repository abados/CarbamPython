...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QSlider, QLabel, QGridLayout, QWidget
...
sacandary_video_port = config["sacandary_video_port"]
arduino_port = config["arduino_port"]


print(f"Main Video Port: {main_video_port}")
print(f"Secondary Video Port: {sacandary_video_port}")
...
class RemoteDrivingApp(QMainWindow):
    def __init__(self):
        ...
        # Inside the __init__ method, add this:
        ...
        # self.send_arm_timer = QTimer()
        # self.send_arm_timer.timeout.connect(self.send_arm_status)  # Continuously process keys
        # self.send_arm_timer.start(500)  # Update every 20ms
        # # Layouts
    ...
    def process_keys(self):
        ...
        if Qt.Key_Z in self.keys_pressed:  # Increase brake
            ...
            self.brake_reset_enable = True

        if Qt.Key_Left in self.keys_pressed:  # Decrease steering
            steering_reset_enable = False
            step = int(np.interp(abs(self.steering_slider.value()), [0, 500], [10, 50]))  # Convert step to integer
        ...
        if Qt.Key_Right in self.keys_pressed:  # Increase steering
            steering_reset_enable = False
            step = int(np.interp(self.steering_slider.value(), [0, 500], [10, 50]))  # Convert step to integer
        ...
        if self.steering_reset_enable:
            if abs(self.steering_slider.value())>7:
                if self.steering_slider.value() < 0:
                    self.steering_slider.setValue(self.steering_slider.value() + 7)
    ...
    def toggle_arm(self):
        ...
        # Send ARM/DISARM status to motor 6
        self.send_to_arduino(6, arm_value)

    def send_to_arduino(self, motor_id, value):
            """
            Send a command to the Arduino in the format 'motorID:value'.
            """
            if self.serial_connection:
                ...
                self.log_message("Arduino connection not established.")

    def send_all_motor_values(self):
        """Send all motor values and fail-safe state to the Arduino as a vector."""
        if self.serial_connection:
            # Collect all motor values and the fail-safe state
            ...
            brake_value = np.interp(self.brake_slider.value(), [0, 100], [self.min_brake, self.max_brake])
            ...
            # Send the vector to the Arduino
            self.serial_connection.write(vector_message.encode())
    ...
    def start_video_stream(self):
        ...
        if not self.video_capture.isOpened():
            self.log_message("Failed to open video stream.")
            return
        ...
        # Set FPS
        self.video_capture.set(cv2.CAP_PROP_FPS, config["fps"])

        # Reduce buffering if supported
        self.video_capture.set(cv2.CAP_PROP_BUFFERSIZE, 1)

        self.timer.start(1000 // config["fps"])  # Adjust timer interval to match FPS
    ...
    def toggle_secondary_camera(self):
        if self.secondary_window.isVisible():
            ...
            self.secondary_video_capture = cv2.VideoCapture(sacandary_video_port)
